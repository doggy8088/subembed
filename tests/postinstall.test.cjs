...
test('maps supported platforms to release artifacts', () => {
  ...
  assert.equal(releaseBaseUrl('1.2.3'), 'https://github.com/doggy8088/subembed/releases/download/v1.2.3');
});
...
test('builds the expected release asset URL list', () => {
  ...
  assert.ok(urls.includes('https://github.com/doggy8088/subembed/releases/download/v9.9.9/subembed-x86_64-unknown-linux-gnu.tar.xz'));
  assert.ok(urls.includes('https://github.com/doggy8088/subembed/releases/download/v9.9.9/subembed-x86_64-pc-windows-msvc.zip.sha256'));
});
